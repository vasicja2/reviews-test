...
public class Controller {
	 ...
	 */
	public void createReview(String typeOption, String title, String authorName, String reviewerName) {
		...
		review = new Review(type, title, authorName, reviewerName);
	}
		...
		for (String id : input.keySet()) {
			if (id.endsWith("hidden")) {
				...
				for (String s : input.get(id)[0].split(" ")) {
					...
					int pos = Integer.parseInt(s.split("\\.")[2]);
					
					par.removeSentence(clusterID, pos);
				}
				continue;
			}
	...
	/**
	 * Returns a list of sentences to each paragraph.
	 * @return
	 */
	...
	/**
	 * Updates the sentence database.
	 * @param input: Key - ID of a standard paragraph; Value - exact text to be saved to the CSV file of the database.
	 */
	...
	/**
	 * Returns the final evaluation of the thesis. If the thesis has not been fully evaluated, returns 0.
	 * @return
	 */
