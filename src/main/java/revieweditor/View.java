...
public class View {
	...
	/**
	 * This is used for testing deployment on Heroku.
	...
	private void requestFrom0(Request request, Controller ctl) {
		...
		String reviewerName = request.queryParams("reviewer");
		String authorName = request.queryParams("author");
		
		if (!ctl.reviewCreated()) 
			ctl.createReview(type, title, authorName, reviewerName);
		else {
			ctl.changeTitle(title);
			ctl.changeReviewType(type);
			ctl.changeReviewerName(reviewerName);
			ctl.changeAuthorName(authorName);
		}
	}
	...
	private void homepage() {
    	...
    	post("/", (request, response) -> {    		
    		...
    		if (request.queryParams().contains("reset")) {
    			ctl.reset();
    			request.session().attribute("completed", 0);
    			response.redirect("/");
    			return new ModelAndView(new HashMap<>(), layout);
    		...
    		if (ctl.reviewCreated()) {
    			title = ctl.getReviewTitle();
    			reviewer = ctl.getReviewerName();
    			author = ctl.getAuthorName();
    		}
	...
	private void step1() {
    	...
    	get("/step1-keywords.js", (request, response) -> {
    		...
    		model.put("kwtypes", ctl.getAvailableKeywords());
	...
	private void step2() {
    	...
    	post("/questions", (request, response) -> {
    		...
    		model.put("pageStyle", "styles/step2.css");
    		model.put("stepNo", "2");
    		model.put("scripts", scripts);
    		model.put("title", ctl.getReviewTitle());
	...
	private void step3() {
    	...
    	get("/step3-clusters.js", (request, response) -> {
    		...
    		model.put("finalGrade", ctl.getFinalGrade());
	...
	private void step4() {
    	...
    	post("/save", (request, response) -> {    		
    		...
    		model.put("author", ctl.getAuthorName());
    		model.put("paragraphs", ctl.getUsedParagraphs());
